...
|►►►boneWeights
|0x2090
|Skin bone ID and weights at each vertex (excluding as many last vertices as the duplicate vertices count) -- byte max number of bones; count * 1..255 byte bone ID (0 indicating no bone); count * 0..1 weight mapped to 0..255
...
|►►►skinBones
|0x1053
|Bone definitions -- byte number of bones, for each bone: nodeID or nodeName referencing a node; 4x4 32-bit float inverse bind matrix (relative to skin bind matrix)
...
|►►animationTrack
|0xA100
|An animation frame track -- nodeID or nodeName reference; (uint) number of keys; number of keys * (uint) frameNumber; 1-byte bool looping flag
